...
# =============================================================================
    ...
    # 回测
    nav, nav_relative, long_portion = model.backtest(merge_ls_factor, '2011-01-01', '2021-09-30')
    ...
    monthly_return.iloc[0,:] = monthly_close.iloc[0,:] / close.iloc[0,:] - 1
    monthly_return['month'] = monthly_return.index.month
    monthly_return['year'] = monthly_return.index.year
    monthly_return_pivot = monthly_return.pivot(index='year', columns='month')['多头/基准']
    monthly_return_pivot.columns=['一月','二月','三月','四月','五月','六月',
                                  '七月','八月','九月','十月','十一月','十二月']
    
    yearly_close = close.resample('Y').last()
    yearly_return = yearly_close.pct_change()
    yearly_return.iloc[0,:] = yearly_close.iloc[0,:] / close.iloc[0,:] - 1
    
    monthly_return_pivot['年化收益'] = yearly_return.loc[:,'多头/基准'].values        
